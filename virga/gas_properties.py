""" Gas phase properties of cloud forming species """
...
def TiO2(mw_atmos, mh=1, gas_mmr = None):
    ...
    density of gas cgs
    """
    # if mh != 1: raise Exception("Alert: No M/H Dependence in TiO2 Routine. Consult
    # your local theorist to determine next steps.")
...
def CH4(mw_atmos , mh = 1,gas_mmr = None):
    ...
    density of gas cgs
    """
    # if mh != 1: raise Exception("Alert: No M/H Dependence in CH4 Routine. Consult your
    # local theorist to determine next steps.")
    #ch4 is linearly dependent on m/h at cold temperatures but scales as m^0 at hotter
    # temps where co is dominant but we can assume a linear dependence here
    if isinstance(gas_mmr, type(None)):
        gas_mmr = 4.9e-4 * mh
...
def NH3(mw_atmos, mh = 1, gas_mmr = None):
    ...
    density of gas cgs
    """
    # if mh != 1: raise Exception("Alert: No M/H Dependence in NH3 Routine. Consult your
    # local theorist to determine next steps.")
    if isinstance(gas_mmr, type(None)):
        gas_mmr = 1.34e-4 * mh
...
def H2O(mw_atmos, mh = 1, gas_mmr = None):
    ...
    density of gas cgs
    """
    # if mh != 1: raise Exception("Alert: No M/H Dependence in H2O Routine. Consult your
    # local theorist to determine next steps.")
    if isinstance(gas_mmr, type(None)):
        gas_mmr = 7.54e-4 * mh
    gas_mw = 18.
...
def Fe(mw_atmos,  mh = 1,gas_mmr = None):
    ...
    Notes
    -----
    .. [1] Morley C.~V., Mukherjee S., Marley M.~S., Fortney J.~J., Visscher C., Lupu R.,
           Gharib-Nezhad E., et al., 2024, ApJ, 975, 59. doi:10.3847/1538-4357/ad71d5

    """
    # if mh != 1: raise Exception("Alert: No M/H Dependence in Fe Routine. Consult your
    # local theorist to determine next steps.")
    if isinstance(gas_mmr, type(None)):
...
def KCl(mw_atmos, mh = 1, gas_mmr = None):
    ...
    Density of gas cgs
    """
    #if mh != 1: raise Exception("Alert: No M/H Dependence in Fe Routine. Consult your local
    # theorist to determine next steps.")

    gas_mw = 74.5
    # mh dependence from Morley12
    # mh_values = np.array([0,0.3,0.5,0.7,1.0,1.7,2.0,2.5])
    # gas_mmrs = np.array([2.2627E-07, 2*2.2627E-07, 3.1*2.2627E-07,5*2.2627E-07, 2.1829E-06,
    # 8.1164E-06,2*8.1164E-06, 3*2*8.1164E-06])
...
def MgSiO3(mw_atmos, mh = 1, gas_mmr = None):
    ...
    """
    gas_mw = 100.4
    # if mh != 1: raise Exception("Alert: No M/H Dependence in MgSiO3 Routine. Consult
    # your local theorist to determine next steps.")
    if isinstance(gas_mmr, type(None)):
...
def Mg2SiO4(mw_atmos, mh = 1,gas_mmr = None,):
    ...
    """
    # if mh != 1: raise Exception("Alert: No M/H Dependence in Mg2SiO4 Routine. Consult
    # your local theorist to determine next steps.")
    if isinstance(gas_mmr, type(None)):
...
def Cr(mw_atmos, mh=1,gas_mmr=None):
    ...
    if isinstance(gas_mmr, type(None)):
        ...
        #else:
        #    raise Exception("Chromium (Cr) gas properties are only available for 1, 10,
        #    and 50xSolar")
...
def Al2O3(mw_atmos, mh = 1, gas_mmr = None):
    ...
    Notes
    -----
    .. [1] Gao, Peter, et al. "Aerosol composition of hot giant exoplanets dominated by
           silicates and hydrocarbon hazes." Nature Astronomy (2020): 1-6.
    .. [2] Lodders, Katharina. "Solar system abundances of the elements." Principles and
           perspectives in cosmochemistry. Springer, Berlin, Heidelberg, 2010. 379-417.
    .. [3] Morley C.~V., Mukherjee S., Marley M.~S., Fortney J.~J., Visscher C., Lupu R.,
           Gharib-Nezhad E., et al., 2024, ApJ, 975, 59. doi:10.3847/1538-4357/ad71d5
    """
    #if mh != 1: raise Exception("Alert: No M/H Dependence in Al2O3 Routine. Consult your
    # local theorist to determine next steps.")
    if isinstance(gas_mmr, type(None)):
...
Proc. 16, 379 (2010)
    """
    #if mh != 1: raise Exception("Alert: No M/H Dependence in Na2S Routine. Consult
    # your local theorist to determine next steps.")
    if isinstance(gas_mmr, type(None)):
        gas_mmr = 3.34e-6 * mh
    gas_mw = 78.05
...
def CaTiO3(mw_atmos, mh = 1, gas_mmr=None):
    ...
    Density of gas cgs
    """
    #if mh != 1: raise Exception("Alert: No M/H Dependence in CaTiO3 Routine.
    # Consult your local theorist to determine next steps.")
    if isinstance(gas_mmr, type(None)):
        gas_mmr = 2.51e-6 * mh
    gas_mw = 135.745
    gas_mmr = gas_mmr * (gas_mw/mw_atmos)
...
def CaAl12O19(mw_atmos, mh = 1,gas_mmr=None):
    ...
    Density of gas cgs
    """
    # if mh != 1: raise Exception("Alert: No M/H Dependence in CaAl12O19
    # Routine. Consult your local theorist to determine next steps.")
    if isinstance(gas_mmr, type(None)):
        gas_mmr = 8.80e-7 * mh
    gas_mw = 667.843
    gas_mmr = gas_mmr * (gas_mw/mw_atmos)
...
def SiO2(mw_atmos, mh=1, gas_mmr = None):
    ...
    density of gas cgs
    """
    #if mh != 1: raise Exception("Alert: No M/H Dependence in SiO2 Routine.
    # Consult your local theorist to determine next steps.")
    if isinstance(gas_mmr, type(None)):
        gas_mmr = 60.3e-6 * mh
    gas_mw = 60
