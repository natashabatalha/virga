...
def SiO2(mw_atmos, mh=1, gas_mmr = None):
    """Defines properties for SiO2 as condensible
    ...
    gas_mmr : float , optional
        Gas mass mixing ratio.
        None points to the default value of : 60.3e-6
    mh : float , optional
        Metallicity, Default is 1=1xSolar
...
def H2S(mw_atmos, mh = 1, gas_mmr = None):
    """Defines properties for H2S as condensible
    ...
    density of gas cgs
    """
    #if mh != 1: raise Exception("Alert: No M/H Dependence in H2S Routine. Consult your local theorist to determine next steps.")
    if isinstance(gas_mmr, type(None)):
        gas_mmr = 1.318e-5 * mh
    gas_mw = 34.08
    gas_mmr = gas_mmr * (gas_mw/mw_atmos)
