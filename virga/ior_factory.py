""" IOR handling functions """
# pylint: disable=R0912,R0913,R0914,R0915,C0103,R0917
...
def configure_ior_choices():
    """ return ior configuration choices """
    return {
    "Al2O3":{
...
def create_ior_db(hitran_dir, output_dir=None,virga_dir=None, lxmie_dir=None,
    ...
    if isinstance(ior_config, str):
        if 'default' in ior_config:
            ...
            raise Exception('The only string choice for ior_config is default. Please '
                            'select default or input a dict')
    elif isinstance(ior_config, dict):
        config = ior_config
    else:
        raise Exception('Not an allowable input for ior_config. Must be either string type '
                        'defualt or modified dict from configure_ior_choices')
    ...
    for i, imol in enumerate(avail):
        ...
        if 'LX-MIE' in config[imol]['url']:
            ...
            if imol=='TiO2':
                hitfile = [hitfile[-1]]
            if imol=='SiO2':
                hitfile = ['/'.join(hitfile[0].split('/')[0:-1])+ '/zeidler_sio2_928k_ab.dat']
            if len(hitfile) > 0:
                skiprows = 5
                tryread=True
                #little hack to read in hitran even though there are different numbers of
                # skipped rows
                while tryread:
                    try:

                        read_hit = pd.read_csv(
                            os.path.join(hitfile[0]), sep=r'\s+', skiprows=skiprows
                        )
            ...
            if not isinstance(files_lxmie,type(None)):
                ...
                if imol=='Fe':
                    kitz = [ikit for ikit in files_lxmie if imol+'.' in ikit ]
                if imol=='MgSiO3':
                    kitz = [ikit for ikit in files_lxmie if 'MgSiO3_amorph_sol-gel' in ikit ]
                for ikit in kitz:
                    readin_LX=pd.read_csv(ikit,sep=r'\s+', skiprows=3, names=['um','n','k'])
                    print('loglog-369')
                    fn[i].loglog(
                        readin_LX['um'].values, readin_LX['n'].values,
                        color='grey',linewidth=6
                    )
                    fk[i].loglog(
                        readin_LX['um'].values, readin_LX['k'].values,
                        color='grey',linewidth=6
                    )
        ...
        #hitran or other raw non-processed data
            ...
            fn[i].loglog(
                datasets['n']['um'].values, datasets['n']['n'].values,
                color='black', marker='o'
            )
            fk[i].loglog(
                datasets['k']['um'].values, datasets['k']['k'].values,
                color='black', marker='o'
            )
