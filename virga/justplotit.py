""" Easy plotting routines for quick plots """
# pylint: disable=R0912,R0913,R0914,R0915,R0917
# pylint: disable=C0103,C0302,C0201
...
from bokeh.models import (LinearColorMapper, LogTicker, BasicTicker, ColorBar,
                          LogColorMapper, Legend)
...
def pt(out, with_condensation=True,return_condensation=False, **kwargs):
    ...
    **kwargs : kwargs
    ...
    Returns
    -------
    if return_condensation: fig, cond temperature curves, ,cond pressure grid , name of
    all available gases
    else: fig
    """
    kwargs['height'] = kwargs.get('plot_height',kwargs.get('height',300))
    kwargs['width'] = kwargs.get('plot_width', kwargs.get('width',600))
    if 'plot_width' in kwargs.keys():
        kwargs.pop('plot_width')
    if 'plot_height' in kwargs.keys():
    ...
    if with_condensation:
        ...
        for i in range(len(all_gases)):
            fig.line(
                cond_ts[i], cond_p, legend_label=all_gases[i], color=cols[i],
                line_width=line_width[i]
            )

    fig.line(
        temperature,pressure, legend_label='User', color='black', line_width=5,
        line_dash='dashed'
    )
...
def radii(out,gas=None,at_pressure = 1e-3, compare=False, legend=None,
    ...
    for j, out_j in enumerate(out):
        ...
        r_g = out_j['mean_particle_r']
        pressure = out_j['pressure']

        nl = find_nearest_1d(pressure,at_pressure)

        rmin = out_j['scalar_inputs']['rmin']
        rmax = out_j['scalar_inputs']['rmax']
        nrad = out_j['scalar_inputs']['nrad']
        log_radii = out_j['scalar_inputs']['log_radii']
        if log_radii > 0:
        ...
        sig = out_j['scalar_inputs']['sig']
        ndz = out_j['column_density']
        ...
        if gas is not None:
            ...
            if len(ndz_gas)>0 :
                ...
                print(gas + " did not condense. Choose another condensate.")
                sys.exit()
        ...
        # consider all gases
            ...
            for i in range(ndz.shape[1]):
                ...
                ndz_gas = ndz_gas[ndz_gas>0]
                if len(ndz_gas)>0:
                    which_condensed[i] = True


        #take only those that condensed
        N = ndz[:, which_condensed]
        r_g = r_g[:,which_condensed]
        gas_name = list(np.array(out_j['condensibles'])[which_condensed])
        r, _, _, _ = pyeddy.get_r_grid(
            r_min=rmin, r_max=rmax, n_radii=nrad, log_space=logspace
        )
        ...
        dndr = {}
        for i in gas_name:
            if out_j['scalar_inputs']['dist'] == 'lognormal':
                # 1e-4 is microns to cm
                dndr[i]= N[nl,gas_name.index(i)]/r/np.sqrt(2*np.pi)*np.log(sig)*np.exp(
                    - np.log(r/(r_g[nl,gas_name.index(i)]*1e-4))**2/(2*np.log(sig)**2))
            elif out_j['scalar_inputs']['dist'] == 'gamma':
                gamma_A = out_j['scalar_inputs']['gamma_A']
        ...
        if j==0:
            p1 = figure(
                width=p1w, height=p1h, title="Select Pressure Level",
                x_axis_type='log',y_axis_type='log',y_axis_label='Pressure (bars)',
                x_axis_label='Mean Particle Radius (um)',
                y_range=[np.max(pressure),np.min(pressure)])

            p2 = figure(
                width=p2w, height=p2h, title="Particle Distribution at %.0e bars" %at_pressure,
        ...
        for i in gas_name:
            if gas is not None or compare:
                ...
                indx = gas_name.index(i)
            f = p1.line(
                i, 'pressure', source=s1, alpha=1,color=color[np.mod(indx, len(color))],
                line_width=4,legend_label=i,line_dash=lines[j]
            )
            p2.line(
                'r', i, source=s2,color=color[np.mod(indx, len(color))] ,line_width=4,
                line_dash=lines[j]
            )
    ...
    p1.line(
        'horizontal', 'average', source=s1, color='black', line_width=3, line_dash='dashed'
    )
    ...
    **kwargs : kwargs
    ...
    for j, out_j in enumerate(out):
        ...
        if compare:
            if gas is not None:
                f = fig.line(
                    opd_by_gas[:,indx], pressure, line_width=4, legend_label=condensibles[indx],
                    color=Colorblind8[np.mod(j, len(Colorblind8))]
                )
            else:
                for i in range(ngas):
                    f = fig.line(
                        opd_by_gas[:,i], pressure,line_width=4, legend_label=condensibles[i],
                        color=Colorblind8[np.mod(j, len(Colorblind8))], line_dash=lines[i]
                    )
            legend_it.append((legend[j], [f]))
        else:
            if gas is not None:
                fig.line(
                    opd_by_gas[:,indx], pressure, line_width=4, legend_label=condensibles[indx],
                    color=col[j], line_dash=lines[j]
                )
            else:
                for i in range(ngas):
                    fig.line(
                        opd_by_gas[:,i], pressure, line_width=4, legend_label=condensibles[i],
                        color=col[i], line_dash=lines[j]
                    )
    ...
    **kwargs : kwargs
    ...
    for j, out_j in enumerate(out):
        ...
        if compare:
            if gas is not None:
                f = fig.line(
                    cond_mmr[:,i], pressure, line_width=4, legend_label=condensibles[indx],
                    color=Colorblind8[np.mod(j, len(Colorblind8))]
                )
            else:
                for i in range(ngas):
                    ...
                    f = fig.line(
                        cond_mmr[:,i], pressure, line_width=4, legend_label=condensibles[i],
                        color=Colorblind8[np.mod(j, len(Colorblind8))], line_dash=lines[i]
                    )
            legend_it.append((legend[j], [f]))
            ...
            if gas is not None:
                fig.line(
                    cond_mmr[:,indx], pressure, line_width=4, legend_label=condensibles[indx],
                    color=Colorblind8[np.mod(j, len(Colorblind8))], line_dash=lines[j]
                )
            else:
                for i in range(ngas):
                    fig.line(
...
def find_nearest_1d(array,value):
    """ find nearest value in 1d array """
...
def pressure_fig(**kwargs):
    """ Create pressure figure elements """
...
def plot_fsed(pressure, z, scale_height, alpha, beta, epsilon=1e-2, pres_alpha=None, **kwargs):
    ...
    for i in enumerate(alpha):
        for j in enumerate(beta):
            ...
            fig.line(
                fsed, pressure, legend_label=lab, color=col, line_width=5, line_dash='solid'
            )
...
def fsed_from_output(out, labels, y_axis='pressure', color_indx=0, cld_bounds=False,
                     gas_indx=None,**kwargs):
    """ Plot fsed from a given Virga output """
    if type(out)==dict:
        out=[out]
...
def aggregates_optical_properties(aggregate, mieff_dir, d_f_list, min_wavelength,
                                  max_wavelength):

    '''
    Code to plot the optical properties calculated in the .mieff files. for all radii and
    fractal dimensions analysed.

    Plots a 3D graph of Q_ext against wavelength (um) for all fractal dimensions (third axis).

    Set filtered = True if you want a smoother curve (using a savgol filter) for easier
    comparison of different fractals.
    ...
    # assign arbitrary column names for reading in the data (all the columns have
    # different numbers of rows)
    mieff_col_names= ['A', 'B', 'C', 'D']
    mieff_data = pd.read_csv(
        f"{mieff_dir}/{aggregate}.mieff", names=mieff_col_names, header=None, sep=r'\s+'
    )
    ...
    # Go through data for one radius size at a time, extracting the Q_ext and wavelength
    # values and storing them in their respective lists
    for r in range(num_radii):
        # reset lists of wavelength and Q_ext each time we move onto a new radius
        wavelengths=[]
        Q_ext=[]

        for i in range(num_wavelengths):
            # only add this wavelength and Q_ext pair to our lists if it's smaller
            # than the user-defined maximum
            if (mieff_data['A'][r*num_wavelengths+r+i+2] <= max_wavelength
                and mieff_data['A'][r*num_wavelengths+r+i+2]>=min_wavelength):
                # find the individual wavelengths at this radius
                wavelengths.append(mieff_data['A'][r*num_wavelengths+r+i+2])
                # find the individual Q_ext values at each wavelength and at this radius
                # (here the index 'r*num_wavelengths+r+i+2' basically filters the .mieff
                # file to return it to a simple 2D array of "Wavelength, Qsca, Qext,
                # g_asymmetry" for one particular particle size)
                Q_ext.append(mieff_data['C'][r*num_wavelengths+r+i+2])

        wavelength_list_spheres.append(wavelengths)
        Q_ext_list_spheres.append(Q_ext)


    # record the radii used in the spherical .mieff file (not needed for the graph,
    # but useful to print alongside it)
    radii=[]
    for r in range(num_radii):
        # convert all radii from cm (default for the .mieff file) to um as we record them
        radii.append(10000.0*mieff_data['A'][r*num_wavelengths+r+1])

    # -------- AGGREGATES --------
    wavelength_list_of_lists=[]
    Q_ext_list_of_lists=[]

    for d_f_j in enumerate(d_f_list):
        # read in each dataset for aggregate particles one fractal dimension at a time
        mieff_data = pd.read_csv(
            f"{mieff_dir}/{aggregate}_aggregates_Df_{d_f_j:.6f}.mieff",
            names=mieff_col_names, header=None, sep=r'\s+'
        )
        ...
        # Go through data for one radius size at a time, extracting the Q_ext and
        # wavelength values and storing them in their respective lists
        for r in range(num_radii):
            # reset lists of wavelength and Q_ext each time we move onto a new radius
            wavelengths=[]
            Q_ext=[]
            ...
            for i in range(num_wavelengths):
                # only add this wavelength and Q_ext pair to our lists if it's smaller than
                # the user-defined maximum
                if (mieff_data['A'][r*num_wavelengths+r+i+2] <= max_wavelength
                        and mieff_data['A'][r*num_wavelengths+r+i+2] >= min_wavelength):
                    # find the individual wavelengths at this radius
                    wavelengths.append(mieff_data['A'][r*num_wavelengths+r+i+2])
    ...
    # plot each radius in a different colour, from blue (small) to green (large)
    radii_colours = plt.cm.winter(np.linspace(0,1,len(wavelength_list_spheres)))
    ...
    for i in range(len(wavelength_list_spheres)):
        # plot Q_ext for spheres at d_f = 3 (the np.full is just to make the array the same
        # length, but the value for d_f is constant every time we run this line)
        ax.plot(
            wavelength_list_spheres[i], np.full(len(wavelength_list_spheres[i]),3),
            Q_ext_list_spheres[i], color = radii_colours[i], linewidth=0.5
        )

    # AGGREGATES: plot each Q_ext vs wavelength graph, one radius (each list element) and
    # one fractal dimension at a time
    for j in range(len(d_f_list)):
        for i in range(len(wavelength_list_spheres)):
            # plot Q_ext for each aggregate at a constant d_f value (the np.full is
            # just to make the array the same length, but the value for d_f is constant
            # every time we run this line)
            ax.plot(
                wavelength_list_of_lists[j][i],
                np.full(len(wavelength_list_of_lists[j][i]), d_f_list[j]),
                Q_ext_list_of_lists[j][i], color = radii_colours[i], linewidth=0.5
            )
...
def aggregates_wavelength_radius_grid(
        clouds_from_virga_spheres, clouds_from_virga_fractals, aggregate, d_f_list,
        mieff_dir, colors=None
):

    '''

    Code to plot the wavelength-radius grid that was used the create the .mieff files,
    and then overplot the actual radii for each pressure layer for each fractal dimension,
    so that we can check that the radii we are using actually have
    ...
    clouds_from_virga_fractals: dict
        array of dicts with output from virga (all of the virga models for each
        fractal dimension)
    aggregate: string
        chemical species used in model
    d_f_list: array
    ...
    Returns
    -------
    Wavelength-radius grid from .mieff files, overplotted with radii for each pressure
    layer for each fractal dimension.
    ...
    # read in the dataset for spherical particles
    # assign arbitrary column names for reading in the data (all the columns
    ...
    for i in range(num_radii):
        # convert all radii from cm (default for the .mieff file) to um as we record them
        list_of_sphere_radii.append(10000.0*mieff_data['A'][i*num_wavelengths+i+1])

    print(f'For the {aggregate}.mieff file (spheres):')
    print(f'\n\t Wavelengths are between {list_of_sphere_wavelengths[num_wavelengths-1]:.3f} '
          f'--> {list_of_sphere_wavelengths[0]:.3f} um in {len(list_of_sphere_wavelengths)} '
          f'intervals.')
    print(f'\t Radii are between {list_of_sphere_radii[0]} --> '
          f'{list_of_sphere_radii[num_radii-1]} um in {len(list_of_sphere_radii)} intervals.')
    ...
    # -------- AGGREGATES --------
    # initialise list to hold all of the radius values (in the cloud only) each
    # fractal dimension
    radius_data_aggregates_list=[]
    # initialise list to hold all of the column_density values (in the cloud only)
    # each fractal dimension
    column_density_data_aggregates_list=[]

    for j in range(len(d_f_list)):

        # ---- SAFETY CHECK -----

        # first, check that we used the same wavelength-radius grid as the spherical
        # model for each fractal dimension (we don't need to use the data from these
        # .mieff files again otherwise -- this is just a safety check)
        mieff_data = pd.read_csv(
            f"{mieff_dir}/{aggregate}_aggregates_Df_{d_f_list[j]:.6f}.mieff",
            names=mieff_col_names, header=None, sep=r'\s+'
        ) # load data

        # extract number of wavelenths and radii used in the grid
        # extract number of wavelengths from header
        num_wavelengths_aggregate = int(mieff_data['A'][0])
        ...
        for i in range(num_radii_aggregate):
            # convert all radii from cm (default for the .mieff file) to um as we record them
            list_of_aggregate_radii.append(
                10000.0*mieff_data['A'][i*num_wavelengths_aggregate+i+1]
            )

        # check wavelength grid is the same as for spheres

        for i, _ in enumerate(list_of_aggregate_wavelengths):
            # if the list of wavelengths is not equal to the version used in the spherical
            # .mieff file, to 6 decimal places
            if abs(list_of_aggregate_wavelengths[i] - list_of_sphere_wavelengths[i]) > 0.000001:
                # print a warning
                print('\n\n WARNING: Wavelength grid is not the same (to 6 dp)!')
                # explain where the error is
                print(f"\n Spherical version: ({num_wavelengths} wavelengths) does not match "
                      f"grid for aggregate {d_f_list[j]:.6f} ({int(mieff_data['A'][0])} "
                      f"wavelengths):\n")
                print(f"\n The exact error occurs for SPHERE wavelength: "
                      f"{list_of_sphere_wavelengths[i]} um, where the AGGREGATE version is: "
                      f"{list_of_aggregate_wavelengths[i]} um.")
                print('\n Exiting code.\n')
                sys.exit() # exit the code

        # check radius grid is the same as for spheres
        for i in enumerate(list_of_aggregate_radii):
            # if the list of radii is not equal to the version used in the spherical
            # .mieff file, to 6 decimal places
            if abs(list_of_aggregate_radii[i] - list_of_sphere_radii[i]) > 0.000001:
                print('\n\n WARNING: Radius grid is not the same (to 6 dp)!') # print a warning
                # explain where the error is
                print(f"\n Spherical version: ({num_radii} radii) does not match grid "
                      f"for aggregate {d_f_list[j]:.6f} ({int(mieff_data['B'][0])} radii):\n")
                print(f"\n The exact error occurs for SPHERE radius: "
                      f"{list_of_sphere_wavelengths[i]} um, where the AGGREGATE version is: "
                      f"{list_of_aggregate_wavelengths[i]} um.")
                print('\n Exiting code.\n')
                sys.exit() # exit the code'''
        ...
        # add this data to a list (one for each fractal dimension)
        column_density_data_aggregates_list.append(reduced_column_density_data)
        # add this data to a list (one for each fractal dimension)
        radius_data_aggregates_list.append(reduced_radius_data)

    # check whether any of the particle radii are outside of the wavelength-radius grid
    warning_flag=0
    # if the radius for one of the fractal dimensions is less than the minimum radius
    # or larger than the maximum for the grid (which we have already checked is the same
    # as the one in the spherical version)...
    for j in enumerate(radius_data_aggregates_list):
        for i in range(len(radius_data_aggregates_list[j])):
            if (radius_data_aggregates_list[j][i] < list_of_sphere_radii[0]
                    or radius_data_aggregates_list[j][i] > list_of_sphere_radii[num_radii-1]):
                # ...print a warning and highlight which fractal dimension and radius caused it
                print(f"WARNING: {d_f_list[j]} has a radius off the grid "
                      f"({radius_data_aggregates_list[j][i]:.6f} um, at pressure "
                      f"{reduced_pressure_data[i]:.6f} bar).\n")
                warning_flag=1
    ...
    ax.vlines(
        list_of_sphere_radii, list_of_sphere_wavelengths[num_wavelengths-1],
        list_of_sphere_wavelengths[0],  colors='lightgrey', linewidth=0.5
    ) # add vertical lines for each radius in grid

    edge_of_grid_color = 'red' # choose colour for box marking outside of grid
    ax.vlines(
        list_of_sphere_radii[0], list_of_sphere_wavelengths[num_wavelengths-1],
        list_of_sphere_wavelengths[0],  colors=edge_of_grid_color
    )
    ax.vlines(
        list_of_sphere_radii[num_radii-1], list_of_sphere_wavelengths[num_wavelengths-1],
        list_of_sphere_wavelengths[0],  colors=edge_of_grid_color
    )
    ax.hlines(
        list_of_sphere_wavelengths[0], list_of_sphere_radii[0],
        list_of_sphere_radii[num_radii-1], colors=edge_of_grid_color
    )
    ax.hlines(
        list_of_sphere_wavelengths[num_wavelengths-1], list_of_sphere_radii[0],
        list_of_sphere_radii[num_radii-1], colors=edge_of_grid_color
    )
    ...
    ax2.scatter(
        radius_data_spheres, column_density_data_spheres, color = color_array[0],
        label= 'Spheres', s=marker_size
    )

    # AGGREGATES: add scatter plot of radii and 'column_density' for each pressure layer
    # of cloud
    for j in enumerate(d_f_list):
        ax2.scatter(
            radius_data_aggregates_list[j], column_density_data_aggregates_list[j],
            color = color_array[j+1], label= f'{d_f_list[j]:.1f}', s=marker_size
        )
...
def aggregates_pressure_vs_number_density(
        clouds_from_virga_spheres, clouds_from_virga_fractals, d_f_list, colors=None,
        min_pressure=None, max_pressure=None
):

    '''

    Code to plot the particle densities at each radius and at each pressure layer for
    aggregates of each fractal dimension. This basically allows you to see the relative
    numbers and radii of the particles that form in each layer, for each fractal dimension.
    ...
    Returns
    -------
    Figure with the particle densities at each radius and at each pressure layer for
    aggregates of each fractal dimension
    ...
    # set maximum and minimum marker sizes (on the screen) - these are arbitrary,
    # and we can scale them to whatever looks good visually
    ...
    for i in range(len(d_f_list)):
        # add each fractal
        radius_data_list.extend(
            clouds_from_virga_fractals[i]['mean_particle_r'].flatten(order='C')
        )
    # add spheres
    radius_data_list.extend(clouds_from_virga_spheres['mean_particle_r'].flatten(order='C'))

    # find th min/max values that we will need to display on-screen
    global_min_r = min(n for n in radius_data_list if n>0) # record the minimum particle
    # size (that is still above zero)
    global_max_r = max(radius_data_list) # record the maximum particle size

    #print(f'\nFINAL RESULTS: Min = {global_min_r}, Max = {global_max_r}')

    # calculate min and max r2 values because these relate linearly to the marker
    # sizes, which are areas
    min_r_squared = global_min_r**2
    max_r_squared = global_max_r**2

    # scale the min/max radii so that they correspond with the min/max marker sizes
    # that we want to show on screen
    marker_size_per_square_radii = ((max_marker_size - min_marker_size) /
                                    (max_r_squared - min_r_squared))
    ...
    for i in range(len(d_f_list)):
        ...
        # first, simply square the radii
        square_radii = clouds_from_virga_fractals[i]['mean_particle_r'].flatten(order='C')**2
        # use derived equation to find the marker area that would give accurately
        # scaled radii on screen
        marker_sizes = (min_marker_size + marker_size_per_square_radii
                        * (square_radii - min_r_squared))
        # make scatter plot
        # legend handle is just there to make the legend markers all the same size
        # (see it's use below)
        legend_handle=ax.scatter(
            clouds_from_virga_fractals[i]['column_density'],
            clouds_from_virga_fractals[i]['pressure'], color = color_array[i+1],
            s=marker_sizes, label= f'{d_f_list[i]:.1f}')

    # plot values for spheres
    # create a list of marker sizes for the radii for spheres
    # first, simply square the radii
    square_radii = clouds_from_virga_spheres['mean_particle_r'].flatten(order='C')**2
    # use derived equation to find the marker area that would give accurately scaled
    # radii on screen
    marker_sizes = (min_marker_size + marker_size_per_square_radii
                    * (square_radii - min_r_squared))

    ax.scatter(
        clouds_from_virga_spheres['column_density'], clouds_from_virga_spheres['pressure'],
        color = color_array[0], s=marker_sizes, label= 'Spheres'
    )
    ...
    # invert the y-axis so that pressure decreases as you go upwards (basically representing
    # altitude)
    ...
    ax.legend(
        list(reversed(handles)), list(reversed(labels)), title='$d_f$', title_fontsize=12,
    ...
    plt.legend(
        handler_map={type(legend_handle): HandlerPathCollection(update_func=update_prop)}
    )
...
def aggregates_pressure_vs_radius(clouds_from_virga_spheres, clouds_from_virga_fractals,
                                  d_f_list, colors=None, width=3, min_pressure=None,
                                  max_pressure=None):
    '''

    Code to plot the particle radius at each pressure layer for aggregates of each
    fractal dimension. Designed originally for diagnostic tests as part for VIRGA
    development by MGL.
    ...
    clouds_from_virga_spheres: dict
        output from virga (spherical model)
    clouds_from_virga_fractals: dict
        array of dicts with output from virga (all of the virga models for each fractal
        dimension)
    d_f_list: array
        list of fractal dimensions analysed
    colors: array (optional)
    ...
    Returns
    -------
    Figure plotting the particle radius at each pressure layer for aggregates of each
    fractal dimension.
    ...
    for i in range(len(d_f_list)):
        ax.plot(
            clouds_from_virga_fractals[i]['mean_particle_r'].flatten(order='C'),
            clouds_from_virga_fractals[i]['pressure'], color = color_array[i+1],
            label= f'{d_f_list[i]:.1f}', linewidth=width
        )

    # plot values for spheres
    ax.plot(
        clouds_from_virga_spheres['mean_particle_r'].flatten(order='C'),
        clouds_from_virga_spheres['pressure'], color = color_array[0],
        label= 'Spheres', linewidth=width
    )
    ...
    # invert the y-axis so that pressure decreases as you go upwards (basically
