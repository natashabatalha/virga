""" Core functionalities for Virga calculation """
...
def vfall(r, grav, mw_atmos, mfp, visc, t, p, rhop, aggregates, Df, N_mon, r_mon, k0):
    ...
    New vfall function written by SEM and MGL, based on the methods of:
        ...
        - "A Condensation–coalescence Cloud Model for Exoplanetary Atmospheres: Formulation
           and Test Applications to Terrestrial and Jovian Clouds", Ohno and Okuzumi (2017)
        - "Theoretical modeling of mineral cloud formation on super-Earths", K. Ohno (2024)

    We adapt the above to include a broad range of aggregates and atmospheric conditions.
    The general outline of the code is:
        ...
        - Determine the Knudsen number:
            ...
            - If Kn > 10 we are in the Free molecular regime (if the mean free path of the
              gas is large compared to the particle size).

                - Here, use equation 46 of Vahidinia et al. (2024) to adapt the fall
                  velocity of spheres for aggregates of fractal dimension d_f

            - If Kn < 10 we are in the Continuum regime (the gas can be treated a
              continuous fluid)

                - Here, use equation 23 of Ohno and Okuzumi (2017) to calculate vfall for
                  aggregates of any type, with the addition of the Beta slip correction
                  factor (Eq 3.11 of Ohno, 2024). This equation (Eq. 23) is valid for all
                  three of the stokes, slip and turbulent regimes, so there is no need to
                  calculate Reynolds number. It is not strictly designed to work for really
                  low fractal dimensions (e.g. DCLA aggregates with d_f = 1.2), so consider
                  the effects an upper bound for these if used (though aggregates in high
                  atmospheres are not usually in the continuum regime anyway, they are in
                  the Free molecular regime. The use of mobility radius for linear DCLA
                  aggregates was explored, but R_gyr was chosen to remain consistent and
                  continuous as d_f changes.
    ...
    if aggregates:
        ...
        if N_mon is not None:
            ...
            # record the original set value to send to the 'find_N_mon_and_r_mon'
            # function below
        ...
        # the aggregate radius can never be less than the monomer particle size, so
        # if this is the case, just use spheres
        if r <= r_mon:
            # SPHERES
            ...
            # AGGREGATES
            # if fractal prefractor k0 is left unprescribed by user, calculate it here
            # (in the case of fixed r_mon, k0 can change as the number of monomers grows,
            # so it needs to be calculated here, for each new radius)
            if k0 <= 0:
                k0 = calculate_k0(N_mon, Df)

            R_gyr = (N_mon/k0)**(1/Df)*r_mon # calculate radius of gyration
            # use the radius of gyration as the characteristic radius of the particle
            r_c = R_gyr
            # calculate characteristic aggregate density using R_gyr as the characteristic
            # radius (derived from Ohno thesis)
            rho_c = rhop*N_mon*(r_mon/R_gyr)**3
    else:
        # SPHERES
        r_c = r # use the radius of the sphere as the characteristic radius of the particle
        rho_c = rhop # use the bulk density of the monomers as the characteristic density

    # calculate knudsen number; remember to use the CHARACTERISTIC radius here to
    ...
    if knudsen > 10: # Free molecular regime
        ...
        # calculate thermal speed (speed of sound)
        sound_speed = np.sqrt(3*k_b*t/(mw_atmos/N_A))

        # find vfall for solid spheres in the Free Molecular regime
        vfall_r = ((2.0/3.0)*grav*rhop/(sound_speed*rho_atmos))*r

        if aggregates and (r > r_mon):
            # if finding vfall for aggregates, apply equation 46 of Vahidinia et al.
            # (2024) (works in the Free Molecular regime only)
            vfall_r = vfall_r * (R_gyr/r_mon)**(((2*Df)-6)/3)

    # Continuum regime: use Eq. 23 of Ohno and Okuzuki (2017), but with the added Beta
    # slip correction factor as in (Eq. 3.11) of "Theoretical modeling of mineral
    # cloud formation on super-Earths" (2024), K. Ohno
    else:
        # calculate the Beta slip factor
        beta_slip = 1. + knudsen*(1.257 + 0.4*np.exp(-1.1/knudsen))
        vfall_r = ((2.0*beta_slip*grav*(r_c**2)*rho_c/(9.0*visc))
                   * (1.0 + (0.45*grav*(r_c**3)*rho_atmos*rho_c
                             /(54.0*visc**2))**(2.0/5.0) )**(-5.0/4.0))

    # For aggregates (especially where d_f<2), check that the terminal velocity of the
    # aggregate > terminal velocity of a single monomer. If this condition is not true, set
    # the monomer terminal velocity as a lower limit for the aggregate's terminal velocity.
    # This is because the aggregate's cross section is probably being over-approximated,
    # and even for the most linear fractals it should not exceed the sum of monomer
    # cross sections -- see Tazaki (2021) Fig. 4

    if aggregates:
        # if the radius is at least as large as 1 monomer (otherwise we just
        # consider small spheres, not aggregates, so the rule below does not apply)
        if r>r_mon:
            # calculate knudsen number for the monomer; using the monomer radius here
            ...
            if knudsen > 10: # Free molecular regime
                N_A = 6.02e23 # avogadro's number
                k_b = 1.38e-16 # Boltzmann constant (in cgs units)
                # calculate thermal speed (speed of sound) (this may not have been done
                # above, as aggregates may not have been in the Kn>10 regime)
                sound_speed = np.sqrt(3*k_b*t/(mw_atmos/N_A))
                # find vfall for single monomers in the Free Molecular regime
                vfall_monomer = ((2.0/3.0)*grav*rhop/(sound_speed*rho_atmos))*r_mon

            # Continuum regime: use Eq. 23 of Ohno and Okuzuki (2017), but with the added
            # Beta slip correction factor as in (Eq. 3.11) of "Theoretical modeling of
            # mineral cloud formation on super-Earths" (2024), K. Ohno
            else:
                # calculate the Beta slip factor
                beta_slip = 1. + knudsen*(1.257 + 0.4*np.exp(-1.1/knudsen))
                vfall_monomer = ((2.0*beta_slip*grav*(r_mon**2)*rhop/(9.0*visc))
                                 * (1.0 + (0.45*grav*(r_mon**3)*rho_atmos*rhop
                                           /(54.0*visc**2))**(2.0/5.0) )**(-5.0/4.0))

            vfall_r = max(vfall_r, vfall_monomer)
...
def vfall_aggregates(r, grav, mw_atmos, t, p, rhop, D=2, Ragg=None):
    ...
    If aggregrate, the monomer size is given by r and the outer effective radius of the
    aggregrate is Ragg.
...
def vfall_aggregrates_ohno(r, grav,mw_atmos,mfp, t, p, rhop, ad_qc, gas_mw, D=2):
    """
    ...
    atmosphere. ***Requires characteristic radius ragg to have D > or equal 2, i.e., not
    very fluffy aggregrates.
    ...
    # calculate the aggregrate radius based on the fractal dimension, monomer radius, and
    ...
    #if kn > 10:
    #    visc = (1.0/3.0)*rho_atmos*v_thermal*mfp #viscosity of the atmosphere,
    #    appropriate for large Kn (Esptein)
    #elif kn < :
    visc = 5.877e-6 * np.sqrt(t) #in dyne/cm^2 with t in K (via Woitke & Helling 2003)

    vfall_stokes = (2.0/9.0) * beta * grav * ((Ragg)**2) * (drho/visc)
    v_bracket = (1.0 + (((0.45/54.0) * (grav/((visc)**2)) * ((Ragg)**3)
                         * rho_atmos * rho_agg)**(2./5.)))**(-5.0/4.0)
...
def vfall_find_root(r, grav=None, mw_atmos=None, mfp=None, visc=None,
                    t=None, p=None, rhop=None, w_convect=None, aggregates=False,
                    Df=None, N_mon=None, r_mon=None, k0=None):
    """
    ...
    #print(f" p = {p:10.5f}     r = {r:10.5e} cm      w_convect = {w_convect:10.5e}
    # v_fall = {vfall_r:10.5e}     Sum = {vfall_r - w_convect:10.5f}")
...
def solve_force_balance(solve_for, temp, grav, mw_atmos, mfp, visc, t, p, rhop, lo, hi):
    ...
    def force_balance_new(u):
        ...
        raise ValueError('Invalid solve_for value.')
...
def qvs_below_model(p_test, qv_dtdlnp=None, qv_p=None,
    ...
    if qv_gas_name in ['Mg2SiO4', 'CaTiO3', 'CaAl12O19', 'FakeHaze', 'H2SO4', 'KhareHaze',
...
def find_cond_t(t_test, p_test = None, mh=None, mmw=None, gas_name=None, gas_mmr=None):
    ...
    gas_mw, gas_mmr, _ = gas_p_fun(mmw,mh=mh , gas_mmr=gas_mmr)
    #get vapor pressure and correct for masses of atmo and gas
    if gas_name in ['Mg2SiO4', 'CaTiO3', 'CaAl12O19', 'FakeHaze', 'H2SO4', 'KhareHaze',
                    'SteamHaze300K', 'SteamHaze400K']:
...
def moment(n, s, loc, scale, dist="lognormal"):
    ...
    def pdf(r, s, loc, scale, dist):
        ...
        raise ValueError('Dist not supported.')
    ...
    # for quicker result, take int(n) and use lognorm.moment, however, it will be
    # slow for int(n) >= 2
    # n = int(round(n))
...
def calculate_k0(N_mon, Df):
    ...
    if N_mon <= 100:
        # the Tazaki equation below is only valid for BCCA aggregates, and at low monomer
        # numbers (e.g. N = 2 monomers) this is not true.
        # Therefore, for N<=100 monomers, we smoothly transition to k0 = 1 for all d_f
        # value (see Moran et al. 2025 for derivation)
        C1 = (1.448-0.716*Df)/99
        C2 = 1.0 - C1
        k0 = C1 * N_mon + C2

    else:
        # for monomer numbers > 100, find k0 using BCCA aggregate Eq. 2 of "Analytic
        # expressions for geometric cross-sections of fractal dust aggregates",
        # Tazaki (2021), MNRAS 504, 2811–2821
...
def find_N_mon_and_r_mon(N_mon_prescribed, radius, original_N_mon, r_mon):
    ...
    Find the following variables for a given radius. One value should be prescribed by the
    user, but both can change according to the growth model in Figure 2 of
    Moran & Lodge (2025):
    ...
    if N_mon_prescribed == 1:
        # first, test the original N_mon value set by the user (this may have been
        # changed on the last loop, so reset it here)
        N_mon = original_N_mon
        # if we have provided N_mon, calculate r_mon (through conservation of
        # volume between an aggregate and a compact sphere)
        r_mon = radius / np.cbrt(N_mon)

        # check whether this value of N and r would mean monomers are smaller than atoms
        # (our absolute lowest physical limit for monomers)
        if r_mon < 1e-8:
            r_mon = 1e-8 # if so, set r_mon = atomic radius in cm (10^-10 m)...
            # ...and decrease N_mon from the prescribed value -- calculate how many of
            # these tiny monomers would be needed to achieve the desired compact
            # particle radius
            N_mon = (radius / r_mon)**3

    else:
        # alternatively, if we have provided r_mon, calculate N_mon (through conservation
        # of volume between aggregate and compact sphere)
        N_mon = (radius / r_mon)**3
