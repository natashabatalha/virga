""" vapour pressure of cloud forming materials """
...
def TiO2(temp, mh = 1 ):
    ...
    Notes
    -----
    .. [1] Marley M.~S., Saumon D., Visscher C., Lupu R., Freedman R., Morley C.,
           Fortney J.~J., et al., 2021, ApJ, 920, 85. doi:10.3847/1538-4357/ac141d
    """
...
def Cr(temp, mh = 1 ):
    ...
    Notes
    -----
    .. [1] Morley, Caroline V., et al. "Neglected clouds in T and Y dwarf atmospheres." The
           Astrophysical Journal 756.2 (2012): 172.
    """
...
def ZnS(temp,mh = 1 ):
    ...
    .. [1] Morley, Caroline V., et al. "Neglected clouds in T and Y dwarf atmospheres." The
           Astrophysical Journal 756.2 (2012): 172.
    .. [2] Visscher, Channon, Katharina Lodders, and Bruce Fegley Jr. "Atmospheric chemistry
           in giant planets, brown dwarfs, and low-mass dwarf stars. II. Sulfur and
           phosphorus." The Astrophysical Journal 648.2 (2006): 1181.
    """
    mh = np.log10(mh)
...
def NH3(temp, mh = 1 ):
    ...
    Notes
    -----
    .. [1] Lodders, K. & Fegley, B. 1998, The planetary scientist's companion / Katharina
           Lodders, Bruce Fegley.  New York : Oxford University Press, 1998. QB601 .L84 1998
    """
    ...
    if len(tlow) > 0:
        pvap_nh3[tlow] = 10**(6.900 - 1588/temp[tlow])
    if len(thigh) > 0:
        pvap_nh3[thigh] = 10**(5.201 - 1248/temp[thigh])
...
def MgSiO3(temp, mh = 1 ):
    ...
    Notes
    -----
    .. [1] Visscher, Channon, Katharina Lodders, and Bruce Fegley Jr. "Atmospheric
           chemistry in giant planets, brown dwarfs, and low-mass dwarf stars. III. Iron,
           magnesium, and silicon." The Astrophysical Journal 716.2 (2010): 1060.
    """
    mh = np.log10(mh)
...
def KCl(temp, mh = 1 ):
    ...
    Notes
    -----
    .. [1] Morley, C.~V., Fortney, J.~J., Marley, M.~S., Visscher, C., Saumon, D.,
           Leggett, S.~K. 2012. Neglected Clouds in T and Y Dwarf Atmospheres. The
           Astrophysical Journal 756. doi:10.1088/0004-637X/756/2/172
    .. [2] Lodders, K. 1999. Alkali Element Chemistry in Cool Dwarf Atmospheres. The
           Astrophysical Journal 519, 793–801. doi:10.1086/307387
    """
...
def H2O(temp,do_buck = True,mh = 1 ):
    ...
    .. [1] Lodders, K. & Fegley, B. 1998, The planetary scientist's companion / Katharina
           Lodders, Bruce Fegley.  New York : Oxford University Press, 1998. QB601 .L84 1998
    .. [2] Buck, Arden L. "New equations for computing vapor pressure and enhancement factor."
           Journal of Applied Meteorology and Climatology 20.12 (1981): 1527-1532.
    .. [3] Flatau, Piotr J., Robert L. Walko, and William R. Cotton. "Polynomial fits to
           saturation vapor pressure." Journal of Applied Meteorology 31.12 (1992): 1507-1513.
    """
    ...
    if len(temp[t_med])>0:
        if do_buck:
            ...
            pvap_h2o = 10*np.exp( (1.0/(temp*temp))*
...
def Al2O3(temp,mh = 1 ):
    ...
    Notes
    -----
    .. [1] Wakeford, Hannah R., et al. "High temperature condensate clouds in super-hot
    Jupiter atmospheres." Monthly Notices of the Royal Astronomical Society (2016): stw2639.
    """
...
def CaAl12O19(temp,p,mh = 1 ):
    ...
    Jupiter atmospheres." Monthly Notices of the Royal Astronomical Society (2016): stw2639.
    """
    #if mh != 1 : raise Exception("Warning: no M/H Dependence in vapor pressure curve
    # for CaAl12O19")
    mh = np.log10(mh)
    #calculated from wakeford 2017
